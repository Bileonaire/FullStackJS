...
const listli = listUl.querySelector('li');
const listlibb = listli.getElementsByTagName('button');
...
function attachListItemButtons() {
  for (let i = 0; i < lis.length; i += 1) {
    console.log(lis[i].querySelector('button.up'));
    let up = document.createElement('button');
    up.className = 'up';
    up.textContent = 'Up';
    lis[i].appendChild(up);


    let down = document.createElement('button');
    down.className = 'down';
    down.textContent = 'Down';
    lis[i].appendChild(down);

    let remove = document.createElement('button');
    remove.className = 'remove';
    remove.textContent = 'Remove';
    lis[i].appendChild(remove);
  }
}
